...
type SkillQuery struct {
	...
	withReviews  *SkillReviewQuery
	withRatings  *SkillRatingQuery
...
func (_q *SkillQuery) QueryReviews() *SkillReviewQuery {
	query := (&SkillReviewClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *SkillQuery) QueryRatings() *SkillRatingQuery {
	query := (&SkillRatingClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *SkillQuery) WithReviews(opts ...func(*SkillReviewQuery)) *SkillQuery {
	query := (&SkillReviewClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *SkillQuery) WithRatings(opts ...func(*SkillRatingQuery)) *SkillQuery {
	query := (&SkillRatingClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *SkillQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Skill, error) {
	...
	if query := _q.withReviews; query != nil {
		if err := _q.loadReviews(ctx, query, nodes,
			func(n *Skill) { n.Edges.Reviews = []*SkillReview{} },
			func(n *Skill, e *SkillReview) { n.Edges.Reviews = append(n.Edges.Reviews, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withRatings; query != nil {
		if err := _q.loadRatings(ctx, query, nodes,
			func(n *Skill) { n.Edges.Ratings = []*SkillRating{} },
			func(n *Skill, e *SkillRating) { n.Edges.Ratings = append(n.Edges.Ratings, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *SkillQuery) loadReviews(ctx context.Context, query *SkillReviewQuery, nodes []*Skill, init func(*Skill), assign func(*Skill, *SkillReview)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Skill)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(skillreview.FieldSkillID)
	}
	query.Where(predicate.SkillReview(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(skill.ReviewsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *SkillQuery) loadRatings(ctx context.Context, query *SkillRatingQuery, nodes []*Skill, init func(*Skill), assign func(*Skill, *SkillRating)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Skill)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(skillrating.FieldSkillID)
	}
	query.Where(predicate.SkillRating(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(skill.RatingsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
