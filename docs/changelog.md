...
| 日期 | 改动 | 涉及 |
|------|------|------|
| 2026-06-18 | 数据 API 联调 bug 全部修复 + 18接口全量通过：① ooh_data DSN env 未被 viper 读取——config.go Init 显式 `v.BindEnv` 绑定 database.ooh_data.dsn/max_open_conns/max_idle_conns 到 MCAI_DATABASE_OOH_DATA_*（viper.Unmarshal 不为 config 缺失 key 查 AutomaticEnv）；② web.Err HTTP 状态码不透传——中间件裸 return *web.Err 绕过 ctx.Failed 落 echo 默认 500，新增 errcode.EncodeErr（errcode 身份映射业务码）+ middleware/RegisterErrorHandler 注册 e.HTTPErrorHandler 透传 402/403/503；③ sqlx 扫描映射 bug（city.all/lines/stations 500）——repo/city_line_biz.go 四处 SelectContext 匿名结构体字段无 db tag 匹配不上 SQL 别名，补 `db:"<别名>"`；④ transaction_logs id 非空约束——schema 加 `.Default(uuid.New)`。18接口回归 18/18 通过，计费扣费+交易日志写入正常 | backend/config/config.go, errcode/errcode.go, middleware/error_handler.go, biz/data/repo/city_line_biz.go, ent/schema/transaction_log.go, pkg/register.go |
| 2026-06-18 | /apis 接口分类重构为二级结构 + 个人中心新增「我的账户」页：① 接口二级分类——ent data_api_pricing 加 group 字段（一级，默认「车站画像」），migration 000016 加列+原 category「车站画像」改名「画像」作二级，docs handler 返回 groups[].group+subGroups[].category+apis，shared dataDoc 类型加 group/subGroups，Apis 页侧栏改可折叠二级树（一级「车站画像」可展开收起，下分 画像/城市/线路/业态/查询）；② 套餐+余额从 /apis 移到个人中心独立页 /settings/account——新增 Settings/Account/index.tsx（套餐卡片+余额卡片+账户流水+可选套餐列表+刷新），Settings 导航加「我的账户」，App.tsx 加路由；③ 修 shared subscriptionApi 路由（/api/v1/subscription→/api/v1/users/subscription）+ Plan/UserSubscription 类型对齐后端 domain（price_month/price_year/*_token_quota，plan 为 string）+ PricingTable 用 price_month；④ migration 000017 seed plans 三档（basic 0元100积分1并发/pro 99元1000积分3并发/ultra 299元5000积分10并发），坑：plans.created_at 无 DB DEFAULT 需显式 NOW() | ent/schema/data_api_pricing.go, migration/000016+000017, biz/data/handler/v1/docs.go, apps/web/src/pages/Apis/index.tsx, apps/web/src/pages/Settings/Account/index.tsx, packages/shared/src/{api,dataDoc+subscription,wallet}.{ts}, types/subscription.ts, components/pricing/PricingTable.tsx |
| 2026-06-18 | mclaw-deploy 技能升级双通道 SSH：局域网 [REDACTED]:22 为主，公网 [REDACTED] 为辅（局域网不通自动切）；公网握手慢需 `-o ConnectTimeout=25`；加公网访问地址段（Web/API https://[REDACTED]）；同步更新 test-server 记忆 | .claude/skills/mclaw-deploy/SKILL.md |
| 2026-06-18 | 数据 API 迁移开发完成（Java→Go）：① Go 基建——go.mod 加 mysql+sqlx，config 加 OohDataDS，pkg/oohdata 只读连接池，consts 加 TransactionDataApiConsumption，ent schema data_api_pricings + make generate，migration 000014(建表+seed18接口) + ClickHouse 000003 data_api_usage_events；② 车站画像 P0——biz/data 模块(model/repo/usecase/handler/register)，6步聚合+Redis缓存，标签固定SQL循环23表，ApiKeyAuth+billing按次计费中间件(余额预检+402)，errcode 11500段；③ 其余12接口——城市7/线路2/业态2/city-durations，repo+usecase+handler 全套；④ Web API文档页——后端 GET /api/v1/data/docs 公开免鉴权，migration 000015 补18接口文档seed，前端 pages/Apis 三栏布局仿RedFox(暖奶油画布+地铁橙，单价/Key徽章/参数表/响应表/curl/JSON/状态码/导出MD)，Layout+App 路由接入；⑤ 日志系统完善——middleware/logging.go 加 Recover+请求/错误日志，error_verbose 用 %#v 暴露 web.Err 私有字段精确定位。已部署到133(公网 [REDACTED])，docs接口✅通过，数据接口2个问题已定位(ooh_data env映射+web.Err状态码透传) | backend/biz/data/, pkg/oohdata/, middleware/logging.go, ent/schema/data_api_pricing.go, migration/000014+000015, apps/web/src/pages/Apis/, packages/shared/src/api/dataDoc.ts |
