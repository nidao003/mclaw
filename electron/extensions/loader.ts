...
function resolveManifestPath(): string {
  if (app.isPackaged) {
    return join(process.resourcesPath, 'mclaw-extensions.json');
  }
  return join(app.getAppPath(), 'mclaw-extensions.json');
}
...
export async function loadExtensionsFromManifest(): Promise<void> {
  ...
  if (existsSync(manifestPath)) {
    ...
    logger.debug('[extensions] No mclaw-extensions.json found, loading all builtin extensions');
  }
