 ...
 *    reason=symlink-escape source=mclaw-managed ...`
 * warning per offending entry on every start.
 *
 * Common offenders are one-shot install scripts that drop symlinks into:
 *   - ~/.mclaw/skills/<name> -> ~/.agents/skills/<name>
 *   - ~/.mclaw/workspace/skills/<name> -> ~/.mclaw/workspace/.agents/skills/<name>
 *   - ~/.mclaw/skills/<name> -> ~/workspace/<repo>/skills/<name>
 ...
 *   - source dirs: ~/.mclaw/skills and ~/.mclaw/workspace/skills
 ...
 *
 * This is a transitional workaround.  Once mclaw/openclaw#59219 lands and
...
export interface CleanupOptions {
  /** Override for ~/.mclaw/skills (mainly for tests). */
  skillsDir?: string;
  /** Override for ~/.agents/skills (mainly for tests/log context). */
  agentsDir?: string;
  /** Override for ~/.mclaw/workspace/skills (mainly for tests). */
  workspaceSkillsDir?: string;
  /** Override for ~/.mclaw/workspace/.agents/skills (mainly for tests). */
...
export interface PluginRuntimeDepsCleanupOptions {
  /** Override for ~/.mclaw/plugin-runtime-deps (mainly for tests). */
 ...
 *
 * OpenClaw can materialize `~/.mclaw/plugin-runtime-deps/mclaw-*` as a
 ...
 *
 * Scope is intentionally narrow: only immediate cache roots named `mclaw-*`
...
export function cleanupStalePluginRuntimeDeps(
  ...
  for (const entry of entries) {
    if (!entry.isDirectory() || !entry.name.startsWith('mclaw-')) {
      continue;
    }
  ...
  if (result.removed.length > 0) {
    logger.info(
      `[skills-cleanup] Removed ${result.removed.length} stray skill symlink(s) ` +
        ...
        `(workaround for mclaw/openclaw#59219): ` +
        result.removed.join(', '),
    );
