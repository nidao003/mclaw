...
} from './mclaw-image-relay-constants';
...
function getAuthProfilesPath(agentId = 'main'): string {
  return join(homedir(), '.mclaw', 'agents', agentId, 'agent', AUTH_PROFILE_FILENAME);
}
 ...
 *
 * This intentionally reads auth-profiles.json rather than mclaw's provider
...
async function discoverAgentIds(): Promise<string[]> {
  const agentsDir = join(homedir(), '.mclaw', 'agents');
  try {
...
async function readOpenClawJson(): Promise<Record<string, unknown>> {
  return (await readJsonFile<Record<string, unknown>>(MCLAW_CONFIG_PATH)) ?? {};
}
...
async function writeOpenClawJson(config: Record<string, unknown>): Promise<void> {
  ...
  await writeJsonFile(MCLAW_CONFIG_PATH, config);
}
...
export async function removeProviderFromOpenClaw(provider: string): Promise<void> {
  ...
  for (const id of agentIds) {
    const modelsPath = join(homedir(), '.mclaw', 'agents', id, 'agent', 'models.json');
    try {
 ...
 * mclaw-written entries historically only included `{ id, name }`.
 *
...
function healAnthropicMessagesMaxTokensInConfig(config: Record<string, unknown>): boolean {
  ...
  for (const [providerKey, entry] of Object.entries(providers)) {
    ...
    if (ensureAnthropicMessagesProviderDefaults(entry, providerKey)) {
      ...
      modified = true;
      console.log(
        `[mclaw-auth] Ensured anthropic-messages maxTokens defaults for models.providers.${providerKey}`,
      );
    }
  }
 ...
 * The bundled OpenClaw distribution mclaw ships does not register that harness,
 ...
 * (written by earlier mclaw builds that did not pin the runtime) get
 * repaired before the next Gateway reload picks them up. Without this, users
 * who upgrade mclaw while still pointing at an OpenAI provider would keep
...
/**
 * Configure a mclaw-owned OpenAI-compatible image provider.
...
export async function syncOpenAiCompatibleImageRelay(params: {
  ...
  return withConfigLock(async () => {
    ...
    if (!params.enabled) {
      const models = (config.models || {}) as Record<string, unknown>;
      const providers = (models.providers || {}) as Record<string, unknown>;
      if (providers[MCLAW_OPENAI_IMAGE_PROVIDER_KEY]) {
        delete providers[MCLAW_OPENAI_IMAGE_PROVIDER_KEY];
        models.providers = providers;
        config.models = models;
      }
      ...
      const primary = typeof imageGenerationModel?.primary === 'string'
        ...
        : '';
      if (defaults && primary.startsWith(`${MCLAW_OPENAI_IMAGE_PROVIDER_KEY}/`)) {
        delete defaults.imageGenerationModel;
      }
      removePluginRegistrations(config, [MCLAW_OPENAI_IMAGE_PROVIDER_KEY]);
      await writeOpenClawJson(config);
      await removeProviderKeyFromOpenClaw(MCLAW_OPENAI_IMAGE_PROVIDER_KEY);
      if (params.apiKey?.trim()) {
        await saveProviderKeyToOpenClaw(MCLAW_OPENAI_IMAGE_PROVIDER_KEY, params.apiKey.trim());
      }
      return;
    }
    ...
    if (modelIds.length === 0) {
      modelIds.push(MCLAW_OPENAI_IMAGE_DEFAULT_MODEL);
    }
    upsertOpenClawProviderEntry(config, MCLAW_OPENAI_IMAGE_PROVIDER_KEY, {
      baseUrl,
    ...
    ensurePluginRegistrationEnabled(config, MCLAW_OPENAI_IMAGE_PROVIDER_KEY);
    await writeOpenClawJson(config);
...
export function readOpenAiCompatibleImageRelayState(
  ...
  const mclawRelay = readModelsProvider(config, MCLAW_OPENAI_IMAGE_PROVIDER_KEY);
  const relayBaseUrl = typeof mclawRelay?.baseUrl === 'string' ? mclawRelay.baseUrl.trim() : '';
  if (relayBaseUrl) {
    return { enabled: true, baseUrl: relayBaseUrl, providerKey: MCLAW_OPENAI_IMAGE_PROVIDER_KEY };
  }

  // Backward compatibility for mclaw builds that used models.providers.openai
  // for image relay. New saves move to the mclaw-owned provider above.
...
function applyControlUiAllowedOrigins(controlUi: Record<string, unknown>, _port: number): void {
  // Web control UI is intentionally disabled — no browser origins allowed.
  // Only keep existing origins that were manually configured (if any).
  const allowedOrigins = Array.isArray(controlUi.allowedOrigins)
    ...
    : [];
  controlUi.allowedOrigins = allowedOrigins.length > 0 ? allowedOrigins : [];
}

/**
 * Write the mclaw gateway token into ~/.mclaw/openclaw.json.
 */
...
/**
 * Ensure browser automation is enabled in ~/.mclaw/openclaw.json.
 */
...
/**
 * Ensure session idle-reset is configured in ~/.mclaw/openclaw.json.
 *
...
async function updateModelsJsonProviderEntriesForAgents(
  ...
  for (const agentId of agentIds) {
    const modelsPath = join(homedir(), '.mclaw', 'agents', agentId, 'agent', 'models.json');
...
/**
 * Sanitize ~/.mclaw/openclaw.json before Gateway start.
 *
...
export async function sanitizeOpenClawConfig(): Promise<void> {
  return withConfigLock(async () => {
    ...
    const rawConfig = await readJsonFile<Record<string, unknown>>(MCLAW_CONFIG_PATH);
    if (rawConfig === null) {
    ...
    // If a user has manually configured a stricter ~/.mclaw/exec-approvals.json,
    ...
    // Different environments may report either "mclaw-lark" or
    // "feishu-mclaw-plugin" as the runtime plugin id.
    if (typeof plugins === 'object' && !Array.isArray(plugins)) {
      ...
      // ── wecom-mclaw-plugin → wecom migration ────────────────
      const LEGACY_WECOM_ID = 'wecom-mclaw-plugin';
      ...
      // manifest-declared "mclaw-qqbot") from plugins.entries.
      // plugins.allow is left untouched — having mclaw-qqbot there is harmless.
      // The channel config under channels.qqbot is preserved and works
      // identically with the built-in channel.
      const QQBOT_PLUGIN_IDS = ['qqbot', 'mclaw-qqbot'] as const;
      ...
      // mclaw bundles Feishu via the official @larksuite/mclaw-lark
      ...
      // for the feishu channel owner and skips mclaw-lark at runtime.
      ...
      if (isFeishuConfigured) {
        ...
        if (hasCanonicalFeishu && canonicalFeishuId !== 'feishu') {
          ...
          if (bareFeishuIdx !== -1) {
            ...
            console.log('[sanitize] Removed bare "feishu" from plugins.allow (mclaw-lark plugin is configured)');
            modified = true;
          }
          if (pEntries.feishu) {
            delete pEntries.feishu;
            console.log('[sanitize] Removed legacy plugins.entries.feishu (mclaw-lark plugin is configured)');
            modified = true;
          }
        }
      }
      ...
      // mclaw-critical bundled plugins, active provider plugins, and explicitly
