...
import { getMclawDir, getOpenClawConfigDir, getOpenClawEntryPath } from './paths';
...
async function runDoctorCommandWithArgs(
  ...
  logger.info(
    `Running OpenClaw doctor (mode=${mode}, entry="${entryScript}", args="${args.join(' ')}", cwd="${mclawDir}", bundledBin=${binPathExists ? 'yes' : 'no'})`,
  );
  ...
  return await new Promise<OpenClawDoctorResult>((resolve) => {
    const child = utilityProcess.fork(entryScript, args, {
      ...
      env: {
        ...
        // Pin OpenClaw doctor to mclaw's own state directory so diagnostics
        // inspect the same data the gateway uses — never the legacy ~/.openclaw/.
