...
import { ensuremclawOpenAiImagePluginInstalled } from './plugin-install';
...
} from './mclaw-image-generation-runtime';
...
export async function setImageGenerationConfig(
  ...
  return withConfigLock(async () => {
    ...
    // mclaw image generation is configured as one explicit custom endpoint.
    // Keep OpenClaw from appending other authenticated image providers such as
    // minimax-portal/image-01 after the configured mclaw image provider.
...
function resolveOpenAiImageRelayModelId(
  ...
  if (primary) {
    ...
    if (slash > 0 && slash < primary.length - 1) {
      ...
      if (provider === MCLAW_OPENAI_IMAGE_PROVIDER_KEY || provider === 'openai') {
        return primary.slice(slash + 1).trim() || MCLAW_OPENAI_IMAGE_DEFAULT_MODEL;
      }
    }
  }
  ...
  const providerEntry = providers && typeof providers === 'object'
    ? (providers as Record<string, unknown>)[MCLAW_OPENAI_IMAGE_PROVIDER_KEY]
    : null;
  return extractModelIdFromProviderEntry(providerEntry) ?? MCLAW_OPENAI_IMAGE_DEFAULT_MODEL;
}
...
export async function getImageGenerationSettingsSnapshot(): Promise<ImageGenerationSettingsSnapshot> {
  ...
  const relayAuthProvider = relayState.providerKey === 'openai' ? 'openai' : MCLAW_OPENAI_IMAGE_PROVIDER_KEY;
...
export async function applyOpenAiImageRelaySettings(params: {
  ...
  if (imageModelIds.length === 0) {
    imageModelIds.push(MCLAW_OPENAI_IMAGE_DEFAULT_MODEL);
  }
  ...
  if (params.enabled) {
    ensuremclawOpenAiImagePluginInstalled();
  }
}
...
export async function listImageGenerationProvidersFromRuntime(): Promise<ImageGenerationProviderRow[]> {
  ...
  return rows.filter((row) => row.id === MCLAW_OPENAI_IMAGE_PROVIDER_KEY);
}
...
function resolveAgentDirForTest(agentId: string, snapshot: AgentsSnapshot): string {
  ...
  const agentDir = entry?.agentDir || `~/.mclaw/agents/${agentId}/agent`;
