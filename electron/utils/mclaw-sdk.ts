 ...
 *
 * openclaw is NOT in the asar's node_modules — it lives at resources/mclaw/
 * (extraResources).  Static `import ... from 'mclaw/plugin-sdk/...'` would
 ...
 * by name, so `openclawRequire('mclaw/plugin-sdk/discord')` resolves via the
 ...
 *
 * Since mclaw only uses the lightweight normalize / directory helpers, we load
...
import { getMclawDir, getOpenClawResolvedDir } from './paths';

const _openclawResolvedPath = getOpenClawResolvedDir();
const _openclawPath = getMclawDir();
...
const _telegramSdk = loadChannelSdk<TelegramSdk>(
  'mclaw/plugin-sdk/telegram-surface',
...
const _whatsappSdk = loadChannelSdk<WhatsappSdk>(
  'mclaw/plugin-sdk/whatsapp-shared',
