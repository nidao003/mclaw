...
When channel plugin ownership changes between bundled OpenClaw extensions and external `~/.mclaw/extensions/*` installs, mclaw must normalize configuration to one active plugin identity per channel.
...
- when the canonical Feishu plugin is external, mclaw must explicitly disable the bundled `feishu` plugin instead of only removing allowlist entries
