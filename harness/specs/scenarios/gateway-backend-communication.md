...
forbiddenPatterns:
  ...
  - fetch('http://127.0.0.1:18999 in src/**
  - fetch("http://127.0.0.1:18999 in src/**
  - fetch('http://localhost:18999 in src/**
  - fetch("http://localhost:18999 in src/**
  - new WebSocket('ws://127.0.0.1:18999 in src/**
  - new WebSocket("ws://127.0.0.1:18999 in src/**
  - new WebSocket('ws://localhost:18999 in src/**
  - new WebSocket("ws://localhost:18999 in src/**
---

Gateway backend communication covers all mclaw paths that move data between the visual desktop UI and OpenClaw runtime/backend services.
...
Channel/plugin migration behavior is also part of this scenario when mclaw rewrites OpenClaw config before Gateway launch. Upgrades must preserve single-owner channel registration for migrated plugin-backed channels such as Feishu/Lark.
