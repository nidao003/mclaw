...
const PATCH_MARKER = 'mclaw-patched-v2: extract directly to $INSTDIR and fail closed';
const LEGACY_PATCH_MARKER = 'mclaw-patched: extract directly to $INSTDIR';
const LEGACY_CONTINUE_ON_EXTRACT_FAILURE = 'continuing overwrite install anyway';
const FATAL_EXTRACT_FAILURE_DETAIL = 'Failed to extract mclaw files after multiple attempts.';
const ROLLBACK_EXTRACT_FAILURE_DETAIL = 'Restoring previous mclaw installation after failed update';

const PATCHED_EXTRACT_MACRO = [
  '!macro extractUsing7za FILE',
  `  ; ${PATCH_MARKER}.`,
  '  StrCpy $R9 0',
  '  mclaw_extract_attempt:',
  '    IntOp $R9 $R9 + 1',
  '    DetailPrint "Extracting mclaw application files (attempt $R9, please wait)..."',
  '    SetOutPath $INSTDIR',
  '    ClearErrors',
  '    Nsis7z::Extract "${FILE}"',
  '    IfErrors 0 mclaw_extract_done',
  '    ${if} $R9 < 5',
  '      DetailPrint "Releasing file locks before retry..."',
  '      nsExec::ExecToStack \'taskkill /F /T /IM "${APP_EXECUTABLE_FILENAME}"\'',
  '      Pop $0',
  '      Pop $1',
  '      nsExec::ExecToStack \'taskkill /F /IM mclaw-gateway.exe\'',
  '      Pop $0',
  '      Pop $1',
  '      Sleep 3000',
  '      Goto mclaw_extract_attempt',
  '    ${endIf}',
  '    DetailPrint "Failed to extract mclaw files after multiple attempts."',
  '    ${if} $mclawRollbackDir != ""',
  '      IfFileExists "$mclawRollbackDir\\" 0 mclaw_extract_show_error',
  '      DetailPrint "Restoring previous mclaw installation after failed update..."',
  '      SetOutPath $TEMP',
  '      RMDir /r "$INSTDIR"',
  '      Rename "$mclawRollbackDir" "$INSTDIR"',
  '    ${endIf}',
  '  mclaw_extract_show_error:',
...
export function patchNsisExtractTemplate(targetPath = EXTRACT_APP_PACKAGE_NSH) {
  ...
  if (hasStaleExtractPatch(original)) {
    console.warn('[patch-nsis-extract] Stale mclaw extract patch detected; replacing with fail-closed patch.');
