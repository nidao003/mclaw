...
beforeEach(() => {
  tempHome = mkdtempSync(path.join(tmpdir(), 'mclaw-uv-env-'));
  vi.resetModules();
  ...
  vi.doMock('electron', () => ({
    app: {
      ...
      getPath: vi.fn().mockReturnValue(path.join(tempHome, '.mclaw')),
...
describe('uv mirror environment', () => {
  it('writes a mclaw-managed uv.toml under ~/.mclaw and exposes it via UV_CONFIG_FILE', async () => {
    const { getUvMirrorEnv } = await import('@electron/utils/uv-env');

    const env = await getUvMirrorEnv();

    const expectedPath = path.join(tempHome, '.mclaw', 'mclaw', 'uv.toml');
